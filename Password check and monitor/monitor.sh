...
function generate_report {
  ...
  num=$(ls ./reports_dir | wc -l)
  if [ $num -gt $MAXIMUM_REPORTS ]; then
...
function calculate_cpu_usage {
  ...
  oldstime=$(awk -F' ' '{ print $15 }' /proc/$PID/stat)
  oldutime=$(awk -F' ' '{ print $14 }' /proc/$PID/stat)

	#Sleep for time_interval
  sleep $TIME_INTERVAL

	#Now, get the current utime and stime (newutime and newstime) /proc/{pid}/stat
  newstime=$(awk -F' ' '{ print $15 }' /proc/$PID/stat)
  newutime=$(awk -F' ' '{ print $14 }' /proc/$PID/stat)
...
function calculate_mem_usage
{
  ...
  mem_usage=$(awk '/VmRSS/ {print}' /proc/$PID/status | awk '{print $2}')
...
function notify
{
  ...
  mem_usage_int=$(printf "%.f" $2)
	...
	filename=$(ls -t ./reports_dir | head -1)
  if [[ $cpu_usage_int -gt $CPU_THRESHOLD ]] || [[ $mem_usage_int -gt $MEM_THRESHOLD ]]; then
    /usr/bin/mailx -s "Usage Report" $USER < ./reports_dir/$file_name
  fi
}
